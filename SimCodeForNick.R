...
(alphaN<-v_fit$psill[2]/sum(v_fit$psill)) #psill/(nugget+psill) from fitted variogram to ranked counts
...
Ysim <- t(apply(Usim,1,function(x) qzig(x,mu,beta,epsilon,Pi))) # transform uniforms to zigs
Ysim[which(Ysim<epsilon)] <- 0 # Replace the data smaller than epsilon with 0.

str(Ysim)

saveRDS(Ysim,
        file = "simulated_Y.rds")
